...
import { ipcDownloadFile, ipcGetUser, ipcGetChatFolder } from '../ipc/index';
...
/**
 * Слушает tdlib_event и диспатчит в нужные сторы.
            ...
            switch (type) {
                case 'myProfile': {
                    ...
                    if (photo?.id && !isDownloaded) {
                        ipcDownloadFile(photo.id).catch(() => { });
                    }
                    break;
                }
                ...
                case 'updateFile': {
                    if (data.file) {
                        ...
                        if (
                            ...
                            useUiStore.getState().setProfile({
                                ...profile,
                                avatarLocalPath: data.file.local.path,
                            });
                        }
                    }
                    break;
                ...
                case 'updateChatLastMessage': {
                    ...
                    if (chatId) {
                        ...
                        if (data.positions && data.positions.length > 0) {
                            ...
                            const mainPos = data.positions.find(p =>
                                p.list?.['@type'] === 'chatListMain'
                            ) || data.positions[0];
                            if (mainPos?.order) {
