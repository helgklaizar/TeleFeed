...
async def main():
    ...
    messages = await client.get_messages('me', limit=1500)
    ai_msgs = []
    
    for msg in messages:
        if msg.text:
            text_lower = msg.text.lower()
            if 'ии' in text_lower or ' ai' in text_lower or 'нейросеть' in text_lower or 'llm' in text_lower or 'агент' in text_lower:
